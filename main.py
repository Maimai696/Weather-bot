...
def build_single_report(icao):
    ...
    if icao in positions and len(positions[icao]) > 0:
        ...
        for target, _, intent in positions[icao]:
            # 独立获取三项概率
            p_brk = calculate_precise_win_rate(target, tp_peak, curr_t, h, "突破")
            p_def = calculate_precise_win_rate(target, tp_peak, curr_t, h, "防守")
            p_yes = calculate_precise_win_rate(target, tp_peak, curr_t, h, "YES")
            ...
            # 原因推断
            if intent == "突破":
                reason = f"辐射 {rad}W/m² 支撑斜率 {slope_str}，冲刺 {float(target)+1.0} 结算点" if main_arrow == "↑" else f"云量 {cloud}% 导致动能流失，进坑流血中"
            else:
                reason = f"云量 {cloud}% 动能封顶，{target}.0 防线稳固" if main_arrow == "↑" else f"物理斜率 {slope_str} 异常，防线受压"
...
def cmd_start(message):
    bot.send_message(message.chat.id, "✅ 交易雷达 V16.1 三矢箭头修复版上线。\n支持指令：\n/pos ZUCK 30N 防守\n/delpos ZHHH 31N\n/now")
