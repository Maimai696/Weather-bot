...
# 1. 配置中心 (环境与字典)
# ==========================================
BOT_TOKEN = os.getenv('BOT_TOKEN') or "如果Zeabur没配变量，直接在这里填入你的TOKEN"
CHECKWX_API_KEY = os.getenv('CHECKWX_API_KEY') or "如果Zeabur没配变量，直接在这里填入你的CHECKWX_API_KEY"
MY_USER_ID = 6822447850 # 用于未来可能的自动推送，手动指令现已改为动态回复
...
def build_single_report(icao):
    ...
    if icao in positions:
        ...
        for target, _, intent in positions[icao]:
            ...
            pos_lines.append(f"({target}N | **突破:{win_pct}% {arrow}** | **防守:{def_val}% -** | **YES区:{round(100-win_pct,1)}% ↓**) {'🚀 [突破套利]' if intent=='突破' else '🛡️ [防守中]'} {trend_txt}\n原因：{reason}")
            global_pos_list.append(f"({flag} {icao}) {target}N **{intent}** (胜率:{win_pct}% {arrow}) {trend_txt}\n理由：{reason}")
        pos_display = "\n".join(pos_lines)

    report = (f"----------------------\n{flag} **{icao} ({airport_metadata[icao]['name']})**\n"
...
# ==========================================
# 4. 指令执行区 (全部修复为动态回复 message.chat.id)
# ==========================================
...
def cmd_start(message):
    bot.send_message(message.chat.id, "✅ 交易雷达 V15.3 已上线。\n支持指令：\n/watch RKPK - 添加监控\n/pos RKPK 25N 突破 - 录入仓位\n/now - 立即生成战报")

@bot.message_handler(commands=['watch', 'unwatch'])
def cmd_watch(message):
    parts = [p.upper() for p in message.text.split()[1:] if len(p) == 4]
    if not parts: return bot.send_message(message.chat.id, "⚠️ 请输入机场代码，例如: /watch RKPK")
    ...
    if '/watch' in message.text:
        ...
        for i in parts: 
            if i in airport_metadata and i not in watchlist: 
                watchlist.append(i)
                added.append(i)
        bot.send_message(message.chat.id, f"✅ 监控已添加: {', '.join(added)}" if added else "⚠️ 机场未录入或已在列表中")
    else:
        for i in parts:
            if i in watchlist: watchlist.remove(i)
        bot.send_message(message.chat.id, f"✅ 监控已移除: {', '.join(parts)}")

@bot.message_handler(commands=['now'])
def cmd_now(message):
    icaos = [i.upper() for i in message.text.split()[1:] if len(i)==4]
    target_list = icaos if icaos else watchlist
    if not target_list: return bot.send_message(message.chat.id, "⚠️ 监控列表为空，请先使用 /watch 或 /pos 添加")
    
    bot.send_message(message.chat.id, "⏳ 正在抓取物理指标与模型数据...")
    reports, all_pos = [], []
...
def cmd_pos(message):
    ...
    if len(parts) >= 3:
        ...
        bot.send_message(message.chat.id, f"✅ {icao} {target}N {intent} 已录入并自动加入监控")
    else:
        bot.send_message(message.chat.id, "⚠️ 格式错误，标准格式: /pos RKPK 25N 突破")
...
def cmd_delpos(message):
    ...
    if parts:
        ...
        if icao in positions:
            del positions[icao]
            bot.send_message(message.chat.id, f"✅ {icao} 仓位已清空")

if __name__ == "__main__":
    print("🚀 交易雷达 V15.3 交互修复全量版已启动...")
