...
def cmd_delpos(message):
    ...
    if icao in positions:
        initial_len = len(positions[icao])
        # 过滤掉我们要删的那个值
        positions[icao] = [p for p in positions[icao] if p[0] != val_to_del]
        
        # 如果这个机场空了，就把机场本身从字典里移走
        if not positions[icao]:
            del positions[icao]
            
        if icao not in positions or len(positions[icao]) < initial_len:
            bot.send_message(MY_USER_ID, f"✅ 成功平仓！已移除 {icao} 的 {val_to_del}N 记录。")
            handle_cmd(message) # 自动刷新雷达
        else:
            bot.send_message(MY_USER_ID, f"⚠️ 未在 {icao} 找到 {val_to_del}N 的记录。")
    else:
        bot.send_message(MY_USER_ID, f"⚠️ {icao} 目前为空仓。")

if __name__ == "__main__":
    print("🚀 交易雷达 V8.9 (平仓支持版) 启动..."); bot.infinity_polling()
