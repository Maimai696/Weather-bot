...
def build_single_report(icao):
    ...
    tp_peak = max(valid_f + [curr_t + m_val*max(0,14-local_dt.hour)]) if valid_f else curr_t

    # 矩阵
    matrix_lines = []
    base_t = math.floor(tp_peak) - 1
    for i in range(4):
        lvl = base_t + i
    ...
    if icao in positions:
        ...
        for target, direction, intent in positions[icao]:
            ...
            if intent == "突破":
                ...
                elif dist >= 1.5 and (m_val <= -0.1 or local_dt.hour >= 17): this_icon = "🛡️"
...
def handle_cmd(message):
    ...
    bot.send_message(MY_USER_ID, f"=== 全局实时雷达 ({datetime.now().strftime('%H:%M')}) ===\n\n{main_text}{risk_footer}", parse_mode="Markdown")
...
if __name__ == "__main__":
    print("🚀 交易雷达 V8.6 (全功能最终版) 启动..."); bot.infinity_polling()
