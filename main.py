...
def build_single_report(icao):
    ...
    if icao in positions:
        ...
        for target, direction, intent in positions[icao]:
            ...
            prob_memory[mem_key] = win_pct # 更新本次胜率入库
            ...
            if intent == "突破":
                ...
                if curr_t >= real_t: this_icon = "💰"; icao_risks.append(f"💰 {icao}|{target}N 击穿(越过{real_t}稳赢)")
...
def cmd_pos(message):
    ...
    while i < len(parts):
        ...
        if len(p) == 4:
            ...
            # 开新仓时，默认给一个初始记忆，防止第一次报 None 错误
            mem_key = f"{cur}_{val}_{intent}"
            if mem_key not in prob_memory: prob_memory[mem_key] = 0.0 # 占位，下次刷新即显箭头
            i += 2
        else: i += 1
