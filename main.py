...
def cmd_start(message):
    bot.send_message(message.chat.id, "✅ 交易雷达 V15.5 极简信号版已上线。\n支持指令：\n/watch RKPK - 添加监控\n/pos RKPK 25N 突破 - 录入仓位\n/now - 立即生成战报")
    ...
    if '/watch' in message.text:
        ...
        for i in parts: 
            if i in airport_metadata and i not in watchlist: 
                watchlist.append(i)
                added.append(i)
        bot.send_message(message.chat.id, f"✅ 监控已添加 (⚪白灯状态): {', '.join(added)}" if added else "⚠️ 机场未录入或已在列表中")
    else:
        for i in parts:
...
def cmd_pos(message):
    ...
    if len(parts) >= 3:
        ...
        bot.send_message(message.chat.id, f"✅ {icao} {target}N {intent} 已录入，启动红绿灯监控")
    else:
...
def cmd_delpos(message):
    ...
    if parts:
        ...
        if icao in positions:
            del positions[icao]
            bot.send_message(message.chat.id, f"✅ {icao} 仓位已清空，恢复为⚪白灯监控状态")

if __name__ == "__main__":
    print("🚀 交易雷达 V15.5 极简信号版已启动...")
