...
def build_single_report(icao):
    ...
    try:
        ...
        metar_data = r['data'][0]
        curr_t = metar_data.get('temperature', {}).get('celsius', "N/A")
    ...
    divergence_gap = 0.0
    if len(valid_f) >= 2:
        divergence_gap = max(valid_f) - min(valid_f)
        if divergence_gap >= 4.0:
    ...
    if valid_f:
        ...
        # 仅在无极端分歧时，允许 ECMWF 微调中位数
        if not model_divergence and isinstance(e_max, (int, float)) and abs(base_f - e_max) > 1.0:
    ...
    if model_divergence:
        icao_risks.append(f"☢️ **{icao} 超算极度分歧** (最大差值{round(divergence_gap,1)}°C，防范微气候陷阱)")
    ...
    forecast_str = f"预测: G:{g_max} I:{m_max} E:{e_max} (基准:{round(base_f,1)})"
    if model_divergence:
        forecast_str += f"\n🚨 **[模型极度分歧] 差值 {round(divergence_gap,1)}°C, 锁定中位数防御!**"
