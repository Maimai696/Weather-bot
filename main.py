...
def build_single_report(icao):
    ...
    try:
        ...
        def get_env(base_key):
            res = hourly.get(f"{base_key}_ecmwf_ifs") or hourly.get(f"{base_key}_gfs_seamless") or [0]*24
            return res[h_idx] if h_idx < len(res) else 0
            
        cloud = get_env('cloudcover')
        rad = get_env('shortwave_radiation')
        wind = round(get_env('windspeed_10m'), 1)
        rh = get_env('relative_humidity_2m')
    except: pass
    ...
    if icao in positions and len(positions[icao]) > 0:
        ...
        for target, _, intent in positions[icao]:
            ...
            if intent == "突破":
                if main_arrow == "↑": reason = f"辐射 {rad}W/m² 支撑斜率 {slope_str}，克服风阻 {wind}km/h 冲刺 {float(target)+1.0} 结算点"
                else: reason = f"风速 {wind}km/h 与湿度 {rh}% 产生热力阻尼，叠加斜率 {slope_str} 疲软，动能流失中"
            else: 
                if main_arrow == "↑": reason = f"风速 {wind}km/h 与湿度 {rh}% 构成热力阻尼，{target}.0 防线稳固"
                else: reason = f"辐射 {rad}W/m² 持续加热，实时斜率 {slope_str} 强于预期，防线受压"
...
def format_execution_board(all_pos):
    numbered_pos = []
    for idx, p in enumerate(all_pos):
        num = NUM_EMOJIS[idx] if idx < 10 else f"{idx+1}."
        numbered_pos.append(f"{num} {p}")
    return "\n".join(numbered_pos)
    ...
    while True:
        try:
            if watchlist:
                ...
                footer = "----------------------\n🚨 **【全仓风险汇总】 (指令执行板):**\n" + (format_execution_board(all_pos) if all_pos else "-- 无持仓")
...
def cmd_start(message):
    bot.send_message(message.chat.id, "✅ 交易雷达 V16.7 视觉提效版上线。\n支持指令：\n/pos ZUCK 30N 防守\n/delpos ZHHH 31N\n/now")
...
def cmd_pos(message):
    ...
    if not parts: return bot.send_message(message.chat.id, "⚠️ 格式: /pos ZHHH 31N 防守")
    current_icao = None
    added = []
    i = 0
    while i < len(parts):
        ...
        if len(p) == 4 and p.isalpha():
            ...
            if current_icao and i + 1 < len(parts):
                p1 = p
                p2 = parts[i+1].upper()
                intent_options = ['突破', '防守', 'YES']
                
                if p1 in intent_options:
                    intent = p1
                    target = p2.replace('N', '')
                elif p2 in intent_options:
                    intent = p2
                    target = p1.replace('N', '')
                else:
                    i += 1
    ...
    if added: bot.send_message(message.chat.id, "✅ 仓位已精准解析录入:\n" + "\n".join(added))
    else: bot.send_message(message.chat.id, "⚠️ 未识别到有效仓位参数，请检查拼写")
