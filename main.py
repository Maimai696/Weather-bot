...
def build_single_report(icao):
    ...
    for i in range(4):
        ...
        p_def = calculate_precise_win_rate(lvl, tp_peak, current_hour, "防守")
        p_yes = calculate_precise_win_rate(lvl, tp_peak, current_hour, "YES")
    ...
    if icao in positions:
        ...
        for target, direction, intent in positions[icao]:
            ...
            if intent == "突破":
                real_target = target_f + 1.0
                dist = abs(real_target - curr_t)
                if curr_t >= real_target:
                    this_icon = "💰"; icao_risks.append(f"💰 **{icao} | {target}N 击穿** (已越过{real_target}稳赢)")
                elif dist <= 1.2:
                    this_icon = "🎯"; icao_risks.append(f"🎯 **{icao} | {target}N 临界** (胜算:{win_pct}% 距{real_target}仅{round(dist,1)}°)")
                elif win_pct < 40: icao_risks.append(f"🚨 **{icao} | {target}N 风险** (动能不足胜算:{win_pct}%)")
            else: 
                real_target = target_f
                dist = abs(real_target - curr_t)
                if curr_t >= real_target - 0.5:
                    this_icon = "🔴"; icao_risks.append(f"🚨 **{icao} | {target}N 危急** (距防线{real_target}仅{round(dist,1)}°)")
                elif dist >= 1.5 and (m_val <= -0.1 or current_hour >= 17):
                    this_icon = "🛡️"
...
def handle_cmd(message):
    ...
    if not icaos: 
        bot.send_message(MY_USER_ID, "⚠️ 监控列表为空。请先使用 `/watch EGLC` 监控机场，或直接使用 `/pos EGLC 18N` 开仓。", parse_mode="Markdown")
        return
...
def cmd_pos(message):
    ...
    while i < len(parts):
        ...
        if len(p) == 4: 
            ...
            if cur not in watchlist: watchlist.append(cur) # 🌟 自动加入监控列表
            i += 1
        elif cur and 'N' in p:
...
def cmd_delpos(message):
    ...
    if len(parts) == 0:
        bot.send_message(MY_USER_ID, "⚠️ 格式错误。请使用:\n精确删除: `/delpos EGLC 16N`\n清空机场: `/delpos EGLC`", parse_mode="Markdown")
        return
    ...
    if len(parts) == 1:
        if icao in positions:
            del positions[icao]
            bot.send_message(MY_USER_ID, f"✅ 已清空 {icao} 的所有仓位。")
        else:
            bot.send_message(MY_USER_ID, f"⚠️ 未找到 {icao} 的持仓记录。")
    else:
        target_str = parts[1].upper()
        val_to_del = "".join(filter(lambda x: x.isdigit() or x=='.', target_str))
        if icao in positions:
            positions[icao] = [p for p in positions[icao] if p[0] != val_to_del]
            if not positions[icao]: del positions[icao]
            bot.send_message(MY_USER_ID, f"✅ 成功平仓！已移除 {icao} 的 {val_to_del}N 记录。")
        else:
            bot.send_message(MY_USER_ID, f"⚠️ {icao} 目前为空仓。")
...
def cmd_unwatch(message):
    parts = message.text.split()[1:]
    if not parts:
        bot.send_message(MY_USER_ID, "⚠️ 格式错误。请使用: `/unwatch EGLC`", parse_mode="Markdown")
        return
    ...
    if removed:
        bot.send_message(MY_USER_ID, f"✅ 已取消监控: {', '.join(removed)}")
    else:
        bot.send_message(MY_USER_ID, f"⚠️ 未在监控列表中找到对应机场。")

if __name__ == "__main__":
    print("🚀 交易雷达 V9.0 (大满贯指令版) 启动..."); bot.infinity_polling()
